...
function Popular() {
  ...
  return (
    ...
    <div className=" h-screen w-screen py-6 bg-[#2F2424]  relative  font-jua">
        <button
          onClick={() => {
            navigate(-1);
          }}
        ...
        <h4 className=" mb-10  text-2xl text-[#d3cfcf] my-6 ">
 ...
 */}
         <div className={`grid   ${state.name=="Specialized Coffee"?" grid-cols-1 sm:grid-cols-2":' grid-cols-1 md:grid-cols-2 '} gap-4 items-center justify-center py-4 mx-7`}>
         {items &&
                items.map((item) => {
                  return (
                    <div
                   onClick={()=>{
                    ...
                    className={`flex shadow-lg justify-betwee  ${state.name=="Specialized Coffee"?"h-28":'h-32 '} w-full overflow-hidden  bg-[#F4ECE5] border-2 border-[#d3cfcf]/60 rounded-xl border-glass `}
                  >
                    {" "}
                    {
...
<div className=  {`${state.name=="Specialized Coffee"?'flex justify-center border-b-2':''}  `}>
                      <h1 className= {`font-semibold  ${state.name=="Specialized Coffee"?'text-[22px]':'text-[14px]'}   text-[#421500] font-serif ${localStorage.getItem("lan")=='en'?" text-start":"text-end"}`}  >
                      ...
                      item.ingredients == "No ingredients" ? (
                        ...
                        <div className={`space-y-1 whitespace-nowrap  ${localStorage.getItem("lan")=='en'?" text-start":"text-end"} `}>
                          <h1 className="font-thin text-[13px] text-[#A73500]">
                            {lang == "ar" ? "المكونات" : "ingredients"}
                          </h1>
                          <p className= {` text-[12px]  text-[#421500] ${localStorage.getItem("lan")=='en'?" text-start":"text-end"}`}>
                            {lang == "ar"
                      ...
                      <div className=" flex justify-between mt-5 w-full whitespace-nowrap relative ">
                        <div>
                          <h1 className= {` text-[#421500]    ${state.name=="Specialized Coffee"?'text-[18px]':'text-[14px]'}`} >
                            {item.price} SAR
                          </h1>
                        </div>
                        <div>
                          <h1 className={` text-[#A73500]    ${state.name=="Specialized Coffee"?'text-[18px]':'text-[14px]'}`} >
                            {item.calories} CAL
                          </h1>
                        </div>
                      </div>
